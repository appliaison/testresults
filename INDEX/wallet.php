...
<?
$acceptedx = num_accepted_for_this_typename("Wallet");

$rejectedx = num_rejected_for_this_typename("Wallet");	
...
<div id="accordion" class="accord">
	...
	<?
	require 'db.php';
	/* this one checks for active type
    ...
    ." WHERE t.type_name LIKE '%Wallet%' AND t.active = 1 ORDER BY e.mail_id DESC";
	*/
	/* this one doesn't check for active type */
		$sql = "SELECT e.mail_id AS mail_id, e.body_id AS body_id, e.mail_date AS mail_date, "
	. " e.mail_reduced_subject AS mail_reduced_subject, "
	. " e.mail_subjectline AS mail_subjectline, e.status AS status FROM email AS e "
	. " LEFT JOIN type AS t ON t.type_id = e.type_id "
    ." WHERE t.type_name LIKE '%Wallet%'  ORDER BY e.mail_id DESC";
